...
def write_durchen_span(poti_num):
    durchen_span_yml = yaml.safe_dump(durchen_span, sort_keys = False)
    Path(f'./test/google_dict/tengyur_durchen_spans.yml').write_text(durchen_span_yml, encoding = 'utf-8')
...
def get_pecha_durchen():
    ...
    for poti_num, poti_path in enumerate(google_ocr,80):
        print(f'v{poti_num} processing ....')
        vol_text = read_poti_content(poti_path)
        pages = get_pages(vol_text)
        cur_vol_text = get_durchen_span(pages, poti_num, text_id)
